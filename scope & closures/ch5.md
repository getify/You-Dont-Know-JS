...
有赖于它被声明的位置，`bar()` 在 `foo()` 的内部作用域拥有一个词法作用域闭包，闭包为了能使 `bar()` 在以后任意的时刻可以引用这个作用域而保持它的存在。
...
我们将内部函数 `baz` 传递给 `bar`，并调用这个内部函数（现在被标记为 `fn`），当我们这么做时，它在 `foo()` 内部作用域上的闭包就可以通过 `a` 的访问观察到。
...
我们拿来一个内部函数（名为 `timer`）将它传递给 `setTimeout(..)`。但是 `timer` 拥有 `wait(..)` 的作用域上的闭包，实际上保持并使用着对变量 `message` 的引用。

在我们执行 `wait(..)` 一千毫秒之后，要不是内部函数 `timer` 依然拥有 `wait()` 内部作用域上的闭包，它早就会消失了。
...
这么说来，所有函数共享一个指向相同的 `i` 的引用是 *理所当然* 的。循环结构的某些东西往往迷惑我们，使我们认为这里有其他更精巧的东西在工作。但是这里没有。这里根本没有循环，5个超时回调仅仅一个接一个地被声明没有区别。
...
另一种在模块模式上微小但是强大的变化是，指定你所返回的对象来作为公有API：
