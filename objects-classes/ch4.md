...
Actually, `this` is very much like a parameter to a function, but it's an implicit parameter rather than an explicit one. You don't see in the function header anywhere, any signal that `this` is going to be used. You have to read the entire function body to see if `this` appears anywhere.
...
My point is, you the author of code, and all other readers of the code even years or decades in the future, need to be `this`-aware. That's the choice, the burden, you place on the reading of such code.

Be aware of *this* `this` choice. Do it intentionally, and do it in such a way as to produce more benefit in the outcome than burden. Make sure `this` usage in your code *carries its own weight*.
...
The `this` mechanism in JS, paired with `[[Prototype]]` delegation, is an extremely powerful pillar of the language. But as the cliche goes: "with great power comes great responsibility". Anecdotally, even though I really like and appreciate *this* pillar, probably less than 5% of the code I ever write uses it. When I do, it's quite on purpose. But it's not my default, go-to JS mechanism.
