...
在我们开始探讨委托之前，我想先提出一点警告。这种关于 JS 对象 `[[Prototype]]` 和 `this` 函数上下文机制的观点*不是*主流观点。这*不是*框架作者和库使用 JS 的方式。据我所知，你不会在外面找到任何大型应用程序使用这种模式。
...
在第 3 章中，我们看到 `constructor(..)` 作为构建 `class` 实例的主要入口点。但 `constructor(..)` 实际上并不做任何*创建*工作，它只做*初始化*工作。换句话说，在 `constructor(..)` 运行并初始化它时——例如 `this.whatever` 类型的赋值——实例已经被创建了。
...
我已经将 `Coordinates` 定义为一个具体对象，它持有我与设置点坐标（`x` 和 `y`）相关联的一些行为。我还将 `Inspect` 定义为一个具体对象，它持有一些调试检查逻辑，例如 `toString()`。
...
但我认为另一个不太明显的好处是，让对象通过 `this` 上下文动态链接往往会使独立测试程序的不同部分稍微容易一些。
