...
在第三，四章中，我们几次提到了`[[Prototype]]`链，但我们没有讨论它到底是什么。现在我们就详细讲解一下prototypes（原型）。

**注意：** 所有模拟类拷贝行为的企图，也就是我们在前面第四章描述的内容，都被标记为“mixin”，和我们要在本章中讲解的`[[Prototype]]`链机制区完全不同。
...
`[[Prototype]]`引用有什么用？在第三种中，我们讲解了`[[Get]]`操作，它会在你引用一个对象上的属性时被调用，比如`myObject.a`。对于默认的`[[Get]]`操作来说，第一步就是检查对象本身是否拥有一个`a`属性，如果有，就使用它。
...
遮蔽甚至会以微妙的方式隐式地发生，所以要想避免它必须小心。考虑这段代码：
...
在JavaScript中有一种奇异的行为被无耻地滥用了许多年来 *骇* 成某些 *看起来* 像“类”的东西。我们来仔细看看这种方式。
...
这种机制常被称为“prototypal inheritance（原型继承）”（我们很快就用代码说明），它经常被说成是动态语言版的“类继承”。这种说法试图建立在面向类世界中对“继承”含义的共识上。但是 *弄拧*（**意思是：铲平**） 了被理解语义，来适应动态脚本。
...
`Foo.prototype`对象默认地（就在代码段中第一行的声明的地方！）得到一个公有的，称为`.constructor`的不可枚举（见第三章）属性，而且这个属性回头指向这个对象关联的函数（这里是`Foo`）。另外，我们看到被“构造器”调用`new Foo()`创建的对象`a` *看起来* 也拥有一个称为`.constructor`的属性，也相似地指向“创建它的函数”。
