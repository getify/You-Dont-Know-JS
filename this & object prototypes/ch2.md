...
The first thing to note, if you were not already aware, is that variables declared in the global scope, as `a = 2` is, are synonymous with global-object properties of the same name. They're not copies of each other, they *are* each other. Think of it as two sides of the same coin. 
If a variables declared in the global scope, as `var a = 2`, are global scope variables, not the global-object properties.

Secondly, we see that when `foo()` is called, `this.a` resolves to our global-object property `a`. Why? Because in this case, the *default binding* for `this` applies to the function call, and so points `this` at the global object.
