...
| 注意： |
| :--- |
| `let` 关键字是在 ES6（2015 年）中新增的。只有当某个应用程序需要在不支持 ES6 之前版本的 JavaScript 环境中运行时，才需要对代码进行上述转译操作。此处的示例仅为简化说明之用。在 ES6 刚推出时，这类转译的需求相当普遍，但到了 2020 年，需要支持 ES6 之前版本环境的情况已经少了很多。因此，转译所使用的 “目标环境（target）” 就像一个滑动窗口，只有当网站 / 应用程序决定停止支持某些老旧浏览器 / 引擎时，这个窗口才会向上调整。 |
