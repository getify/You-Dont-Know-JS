...
Inversely, for the `&&` operator, if *first operand* can be converted to false, the `&&` expression results in the value of it (`a` or `c`). Otherwise, the `&&` expression results in the value of the *second operand* (`b`). 
