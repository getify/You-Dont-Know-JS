...
In JavaScript, the *type* of the value controls whether that value will be assigned through value-copy or by reference.

## Summary

JavaScript provides object wrappers around primitive values. These object wrappers give the values access to behaviors appropriate for each object sub-type (`String#trim()` and `Array#concat(..)`).
