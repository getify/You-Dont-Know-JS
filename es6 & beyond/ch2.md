...
* If you have an inner function expression that's relying on a `var self = this` hack or a `.bind(this)` call on it, that function expression can probably safely become an `=>` arrow function.
* For function declarations, long multi-statment function expressions, functions which need a lexical name identifier self-reference (recursion, etc.), and `this`-aware functions that otherwise do not rely on `var self = this` or `.bind(this)`, you should probably avoid `=>` and stick with your normal function syntax.
...
In much the same method of reasoning as I just presented for `this`, if you have functions that are doing things like `var args = Array.prototype.slice.call(arguments)` to make a lexical copy of `arguments` for an inner function to reference, that's also a case where `=>` can help. The same also goes for `super`, though you won't have used that prior to ES6 anyway, so refactoring won't likely be the primary motivation.
