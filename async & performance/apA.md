...
Thirdly, sequences can more easily be twisted to adapt to different modes of thinking, such as event/steam/reactive based coding. *asynquence* provides a pattern I call "reactive sequences" (which we'll cover later) as a variation on the "reactive observable" ideas in RxJS ("Reactive Extensions"), which lets a repeatable event fire of a new sequence instance each time. Promises are one-shot-only, so it's quite awkward to express repetitious asynchrony with promises alone.
