...
Типичным решением будет создание обертки (массива или объекта), котора будет хранить множественные значения. Решение рабочие, но может быть довольно неудобно извлекать значения из обертки на каждом шаге промисов.
...
Выглядит лучше лучше! Конечно, вы можете заинлайнить этот трюк, чтобы не создавать дополнительную функцию:
...
```javascript
// `click(..)` свзяывает `"click"` событие с DOM элементом
...
`Promise.wrap()` не создает промис. Он создает функцию, которая создаст промис. В некотором смысле можно считать эту функцию "фабрикой промисов". Я предложил свое навзание - "промисор" [от англ. Promisory ("promise" + "factory")]
...
```
Конечно, пока мы рефакторим `foo()`, чтобы использовать промисор `request()`, мы могли бы превратить `foo()` в промисор и не использовать `betterFoo()`. Делать так или нет, зависит от того, должен ли `foo()` оставаться колбэк-совместимым с другими часятми программы или нет.
...
Если сравнивать асинхронную задачу на колбэках, и цепочку промисов, то промисы несколько медленнее. Однако и подумайте о тех гарантиях, которые дают промисы "из коробки", по сравнению с тем количеством вспомогательного кода, которое придется "городить" руками для колбэков.
...
Еще одна тонкая проблема в том, что промисы делают асинхронным все к чему прикоснутся. Это значит, что синхронные действия будут откладывать переход к следующему шагу (см. Главу 1). Это также означает, что цепочка промисов будет выполнятся медленнее, чем аналогичная цепочка на колбэках.
