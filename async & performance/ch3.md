...
**Примечание:** Паттерн `new Promise (function (..) {..})` называется «раскрывающим конструктором». Передаваемая функция выполняется немедленно и ей передаются два параметра, которые мы назвали `resolve` и `reject`. Это функции выполнения промиса. `resolve(..)` отвечает за `fulfillment`-стадию и `reject(..)` за `rejection`-стадию.
...
Вместо того, чтобы передавать промис в `bar(..)` и `baz(..)`, мы используем промис для контроля выполнения `bar(..)` и `baz(..)` , если это вообще произойдет. Основное различие заключается в обработке ошибок.
...
В мире промисов важная деталь заключается в том, как точно узнать, является ли какое-то значение промисом или нет. Или более точно — будет ли значение вести себя, как промис?
...
Более того, библиотеки или фреймворки могут предоставлять свои собственные промисы, реализация котрых отличается от реализации промисов в `ES6` . Так же, вы вполне можете использовать промисы с библиотеками в старых браузерах, которые не поддерживают промисы.

Итак, было решено, что способ распознать промис (или что-то, что ведет себя как промис) — это определить некую сущность, называемую `«thenable»`. Это любой объект или функция, у которого(-ой) есть метод `then(...)`. Предполагается, что любое такое значение является совместимым с промисом.
...
Но `v` при этом также является связанным через [[Prototype]] с другим объектом `o`, который имеет `then(..)`. Это значит, что проверка «утиной типизацией» покажет, что `v` это `thenabl`e. Нехорошо(
...
Вы, несомненно, заметили, что использование промисов не отменяет использования колбэков. Они просто меняют место вызова колбэка. Вместо того, чтобы передавать колбэк `foo(..)`, мы получаем *нечто* (якобы промис) из `foo(..)`, и мы передаем колбэк этому *нечту*.
...
Надееюсь, теперь в вашем созвании полностью «зарезолвилось» понимание надежности промисов. И что еще более важно у вас появлилось понимание, почему эта надежность настолько важна для создания универсального, поддерживаемого программного обеспечения.
...
Это только первая половина главы о промисах. Глава получилась очень большой и тяжелой для восприятия при чтении разом, поэтому было принято решение о разбиении на две половины.

В следующей части мы более подробно разберем механизм связывание промисов, детальнее изучим понятия `Resolve`, `Fulfill`, and `Reject`, а также изучим основные шаблоны использования промисов. Не переключайтесь.

![Please stand by](https://i.ytimg.com/vi/84qJttZjp94/maxresdefault.jpg)
