...
.then( function(v){
	// запустится после задержки на предыдущем шаге в 100 милисекунд
	console.log( v );	// 42
} );
```
...
Если сравнивать асинхронную задачу на колбэках, и цепочку промисов, то промисы несколько медленнее. Однако и подумайте о тех гарантиях, которые дают промисы "из коробки", по сравнению с тем количеством вспомогательного кода, который придется "городить" руками для колбэков.
...
Еще одна тонкая проблема в том, что промисы делают асинхронным все. Это значит, что синхронные действия будут откладывать переход к следующему шагу (см. Главу 1). Это также означает, что цепочка промисов будет выполнятся медленее, чем аналогичная цепочка на колбэках.
...
Они не избавляются от колбэков, они просто перенаправляют управление ими на надежный механизм, который находится между нами и другой утилитой.
