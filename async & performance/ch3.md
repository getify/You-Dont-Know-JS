...
Если что-то пошло не так, при получении значений `X` или `Y` или при сложении, промис, который возвращает `add(..)`, будет зареджекшен, а второй обработчик ошибок, переданный в `then(..)`, получит значение реджекта.
