...
Начиная со стандарта ES6 появился способ получить *итератор* из *итерируемого* объекта или массива. Для этого *итерируемый* должен иметь функцию, в названием которой является значение ES6-символа `Symbol.iterator`. При вызове такой функции возвращается *итератор*. Хотя это не является обязательным, обычно каждый вызов возвращает новый *итератор*.
