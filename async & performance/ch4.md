...
Внутри `*foo(..)` инструкция `var y = x ..` начинает обрабатываться, и затем она доходит до ключевого слова `yield`. Здесь `*foo(..)` приостанавливает свою работу (прямо во время операции присвоения!) и запрашивает значение у исполняющего кода, которое должно быть передано в `yield`. Затем мы вызываем `it.next( 7 )`, тем самым пробрасывая `7` как результат приостановленного выражения `yield`.
