...
И вот здесь мы можем увидеть разницу между тем, как ведут себя в процессе вызова генератор и обычная функция. `foo(6,7)` выглядит знакомо, однако, в отличии от обычной функции, генератор в этот момент еще не начал выполняться.
